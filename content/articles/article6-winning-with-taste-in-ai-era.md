...
## Context: Building Is Getting Cheap
## Context：做出来正在变便宜

**English:**

AI is making the distance between idea and implementation feel strangely short.

You can open a chat window with a rough product thought and get back a prototype shape. You can ask for architecture options, migration plans, tests, edge cases, and first-pass code. A founder can generate landing pages, sales copy, research questions, pricing pages, dashboards, and internal tools before lunch.

None of this means the work is automatically good.

The output still breaks. The plan still needs judgment. The code still needs review. The product still needs a real user with a real problem.

But the direction is obvious: more people can make more things faster.

That changes the bottleneck.

When making was expensive, the winner was often the person who could execute at all. When making becomes cheap, the winner is the person who can tell what is worth making, what good looks like, and where the current version is quietly wrong.

That is taste.

The annoying part is that I do not fully know how to define taste.

That is also the interesting part.

Taste is not one clean skill. It is not just aesthetics. It is not just product judgment, technical judgment, business judgment, or design judgment. It is a moving mix of all of them, shaped by exposure, decisions, mistakes, role models, and repeated contact with reality.

So the useful question is not whether we can define taste perfectly.

The useful question is whether we can approximate its shape and train toward it.

**中文：**

AI 正在让 idea 到 implementation 的距离变得很短，短到有点不真实。

你打开一个 chat window，丢进去一个粗糙产品想法，就能拿回一个 prototype 的形状。你可以直接要 architecture options、migration plan、tests、edge cases 和 first-pass code。Founder 甚至可以在午饭前生成 landing page、sales copy、研究问题、pricing page、dashboard 和内部工具。

这当然不代表产出自动变好。

输出还是会坏。计划还是需要判断。代码还是要 review。产品最后还是要面对一个有真实问题的真实用户。

但方向已经很清楚：更多人可以更快做出更多东西。

瓶颈因此改变了。

当“做出来”很贵时，赢家常常是那个能执行的人。当“做出来”变便宜时，赢家变成那个能判断什么值得做、什么算好、当前版本哪里悄悄不对劲的人。

这就是 taste。

麻烦的是，我也没法把 taste 定义得特别干净。

但这恰恰是它有意思的地方。

Taste 不是一个单一技能。它不只是审美。不只是产品判断、技术判断、商业判断或设计判断。它是这些东西的移动组合，被 exposure、decision、mistake、role model 和反复接触现实塑形。

所以真正有用的问题不是：我们能不能完美定义 taste？

真正有用的问题是：我们能不能逼近它的形状，然后朝那个方向训练自己？
...
It notices when something is almost right but not sharp enough. It notices when a product feels powerful but confusing. It notices when a technical design is elegant but too clever. It notices when a strategy sounds smart but will not survive the first real customer conversation.

Good taste does not always give you the full answer.

Often it starts as a smell.
...
Taste is what lets you pause in that moment and say: this looks fine, but it is not real yet.

**中文：**

Taste 是在模糊环境里感知质量的传感器。

它会提醒你：这个东西接近对了，但还不够锋利。这个产品看起来强，但用起来乱。这个技术设计很优雅，但太聪明。这个 strategy 听起来漂亮，但遇到第一个真实客户就会碎。
...
Taste 让你能在那个瞬间停下来，说：这东西看着没问题，但它还不是真的。
...
Pick a frontier. AI coding. Agent workflows. AI-native education. Personal software. Vertical AI. New design tools. Anything real enough that people are building it, using it, and arguing about it.

Then take a side.

Not forever. Not as religion. Just seriously enough that your view can be wrong in public.

Advocacy sharpens perception. If you say AI coding is the future, you have to learn which parts of coding AI actually improves, which parts it still fails at, which teams benefit first, which workflows are fake, and which demos hide human labor.

If you say agents will change enterprise software, you have to learn where agents break: permissions, memory, evaluation, latency, trust, handoff, auditability, and unclear ownership.
...
选一个前沿方向。AI coding。Agent workflow。AI-native education。Personal software。Vertical AI。新的设计工具。任何一个足够真实、已经有人在建、有人在用、有人在争论的方向都可以。

然后站队。

不是永远站队。也不是宗教式站队。只是认真到你的观点有可能在现实面前出丑。
...
You have to use the frontier products until they become a little uncomfortable.
...
Then I started copying their work habits. How they prepared for hard conversations. How they decided what deserved urgency. How they communicated uncertainty without sounding lost. How they moved through ambiguity without pretending the ambiguity was gone.

Only later did I realize I was not just copying style.

I was borrowing a decision system.
...
后来开始模仿工作习惯。他们怎么准备困难对话。怎么判断什么事情真的 urgent。怎么在不确定时沟通，但又不显得迷失。怎么穿过 ambiguity，同时不假装 ambiguity 已经消失。

再后来才意识到，我模仿的不是风格。

我是在借用一套 decision system。
